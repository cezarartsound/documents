...
void VGA_paint(int x, int y,int w, int h, int r, int g, int b){
	...
	for (yi = y; yi < (y+h); yi++ )
			for (xi = x; xi < (x+w); xi++ ) {
				...
				location = (xi+VGA_vinfo.xoffset) * (VGA_vinfo.bits_per_pixel/8) +
						   (yi+VGA_vinfo.yoffset) * VGA_finfo.line_length;

				//if (VGA_vinfo.bits_per_pixel == 32 ) {
				//	*(fbp + location) = 100;        // Some blue
				//	*(fbp + location + 1) = 200+(x-100)/2;     // A little green
				//	*(fbp + location + 2) = 15-(y-100)/5;    // A lot of red
				//	*(fbp + location + 3) = 0;      // No transparency
				//} else  { //assume 16bpp
					unsigned int t = VGA_get16bpp_color(nr, ng, nb); //r<<11 | g << 5 | b;
					*((unsigned int*)(fbp + location)) = t;
				//}

			}
}

#define VGA_direct_set_pixel(fbp_idx, value) (*((unsigned int*)(fbp + (fbp_idx))) = value)

void VGA_set_pixel(int x, int y, unsigned int color ){
	long int location = (x+VGA_vinfo.xoffset) * (VGA_vinfo.bits_per_pixel/8) + (y+VGA_vinfo.yoffset) * VGA_finfo.line_length;

	VGA_direct_set_pixel(location, color);
...
unsigned int VGA_8bpp_to_16bpp(byte b8){
	int r,g,b;
	/*r = b8>>5;
	g = b8>>2&0b111;
	b = b8&0b11;
...
inline void VGA_fill_rect(int x1,int y1, int x2, int y2, unsigned int color){
	int xi, yi;

	for (yi = y1; yi < y2; yi++) {
		for (xi = x1; xi < x2; xi++) {
			VGA_set_pixel(xi, yi, color);
		}
	}
}
